...
const SHEET_GID = '1888497588';                     // gid của sheet (Xuất)
const SHEET_NAME = 'xg-xuat';                       // Tên sheet tab
...
const TON_SHEET_GID = '1968603689'; // GID trang Tồn
...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbw6lrjrYn3EMjzGCiiRDWbBUCcudzv-IDRgYukUxWtVswHeVXfpM8PT1TwmafPm4RFr/exec';
...
let currentMaVatTuFilter = ''; // Filter state for inventory rolls
...
function setupModalPermissions(modalEl) {
  ...
  const btnAddRollModal = modalEl.querySelector('#btnAddRoll, #btnEditAddRoll, #addRollBtn, #editAddRollBtn');
  if (btnAddRollModal) {
    btnAddRollModal.style.display = isAdmin ? '' : 'none';
  }
...
function openAddDataModal() {
  ...
  if (btnAddRoll) {
    ...
    btnAddRoll.onclick = () => {
      const maVatTu = maVatTuInput ? maVatTuInput.value.trim() : '';
...
function addRollRow(maVatTu = '', cuonId = '', kgValue = '') {
  ...
  tr.innerHTML = `
    ...
    <td>
      <input type="text" class="form-control form-control-sm roll-ma-vt" required
