  ...
  try {
    ...
    if (e.target && e.target.id === 'addDataForm') {
      ...
      try {
        ...
        console.warn('RPC xuat_xg_atomic failed, falling back to direct insert if RPC not found:', rpcErr);
        if (rpcErr.message && (rpcErr.message.includes('function public.xuat_xg_atomic') || rpcErr.message.includes('does not exist'))) {
