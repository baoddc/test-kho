...
async function loadAllData() {
  try {
    ...
    // 1. Tải song song toàn bộ dữ liệu từ 3 bảng trong Supabase
    ...
    // Nếu bảng tồn rỗng, tự động tính toán tồn kho theo logic Cuộn ID (như trang xg-ton-supabase.js)
...
function calculateRollMetrics() {
  ...
  for (const rollId in importsMap) {
    ...
    // Bỏ qua nếu nhập sau kỳ đánh giá (chưa tồn tại trong kỳ)
    if (imp.dateIn > evaluationEndDate) continue;

    // Bỏ qua nếu đã xuất trước kỳ đánh giá (không còn tồn trong kỳ)
    if (exp && exp.dateOut < evaluationStartDate) continue;

    let storageDays = 0;

    if (exp && exp.dateOut <= evaluationEndDate) {
      // Đã xuất trong kỳ: Ngày xuất - Ngày nhập
      storageDays = (exp.dateOut - imp.dateIn) / (1000 * 60 * 60 * 24);
    } else {
      // Chưa xuất (hoặc sẽ xuất sau kỳ): Ngày chốt kỳ (evaluationEndDate) - Ngày nhập
      storageDays = (evaluationEndDate - imp.dateIn) / (1000 * 60 * 60 * 24);
    }
...
function processDataAndCreateCharts() {
  ...
  // Find "Loại nhập" or "Loại xuất" column index (column 5 = index 4)
  const findLoaiNhapColIndex = (headers) => {
    for (let i = 0; i < headers.length; i++) {
      const h = String(headers[i] || '').toLowerCase().trim();
      if (h.includes('loại') || h.includes('loai')) {
        return i;
      }
    }
  ...
  const toDateInputVal = document.getElementById('toDate')?.value;
  let evaluationEndDate = new Date();
  if (toDateInputVal) {
    evaluationEndDate = new Date(toDateInputVal);
  }
  evaluationEndDate.setHours(23, 59, 59, 999);
  ...
  if (tonData && tonData.length > 1) {
    ...
    // Nếu bảng tồn kho trống, tính toán động bằng tổng nhập trừ tổng xuất lũy kế
    let allImportsSum = 0;
    for (let i = 1; i < importData.length; i++) {
      const row = importData[i];
      if (!row || row.length === 0) continue;
      allImportsSum += parseNumericInput(row[importQtyColIndex]) || 0;
    }
    let allExportsSum = 0;
    for (let i = 1; i < exportData.length; i++) {
      const row = exportData[i];
      if (!row || row.length === 0) continue;
      allExportsSum += parseNumericInput(row[exportQtyColIndex]) || 0;
    }
    totalCurrentStock = Math.max(0, allImportsSum - allExportsSum);
  }
