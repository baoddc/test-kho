...
const SHEET_GID = '425790242';                 // gid của sheet (thường là 0 cho sheet đầu)
...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyETkmyoVVYqjvjGoaukEnDNzySnhZsizfK5fKRu73NyfdT_sPqfo4VtfaMVIQd16Eyzw/exec';
...
function openAddDataModal() {
  ...
  for (let i = quantityColIndex + 1; i < headers.length; i++) {
    ...
    // Hide column 12 (Column M) and 'quy đổi' columns
    if (i === 12 || headerName.includes('quy đổi') || headerName.includes('quy doi')) {
      col.style.display = 'none';
...
function openEditDataModal() {
  ...
  if (rowData[12] !== undefined) {
    const parsedM = parseNumericInput(rowData[12]);
  ...
  addEditRollRow(existingCuonId, existingKg, existingM, existingViTri);
  ...
  tr.innerHTML = `
    ...
    <td>
      <input type="number" class="form-control form-control-sm roll-kg" 
             ...
             placeholder="Nhập số kg" value="${kgValue}" required>
    </td>
    <td>
      <input type="number" class="form-control form-control-sm roll-m" 
             step="any" min="0" inputMode="decimal" 
             placeholder="Nhập số m" value="${mValue}" required>
    </td>
    <td>
...
function updateRollTotals() {
  ...
  document.getElementById('totalKg').textContent = totalKg.toFixed(2).replace('.', ',') + ' kg';
...
function addEditRollRow(cuonId = '', kgValue = '', mValue = '', viTri = '') {
  ...
  tr.innerHTML = `
    ...
    <td>
      <input type="number" class="form-control form-control-sm edit-roll-m" 
             step="any" min="0" inputMode="decimal" 
             placeholder="Nhập số m" value="${mValue}" required>
    </td>
    <td>
...
function updateEditRollTotals() {
  ...
  document.getElementById('editTotalKg').textContent = totalKg.toFixed(2).replace('.', ',') + ' kg';
...
document.addEventListener('submit', async (e) => {
  try {
    ...
    if (e.target && e.target.id === 'addDataForm') {
      ...
      // Get roll data (Kg, M, Cuộn ID, Vị trí)
      ...
      if (rollValues.length === 0) {
        alert('Vui lòng nhập đầy đủ Số kg và Số m (> 0) cho ít nhất một cuộn');
        if (submitBtn) {
      ...
      const rowsToAdd = rollValues.map(val => {
        ...
        const maxIdx = Math.max(cuonIdIdx, viTriIdx);
        if (maxIdx !== -1) {
          while (newRow.length <= maxIdx) newRow.push('');
          if (cuonIdIdx !== -1 && val.cuonId) newRow[cuonIdIdx] = val.cuonId;
          if (viTriIdx !== -1 && val.viTri) newRow[viTriIdx] = val.viTri;
        }
    ...
    // ===== EDIT DATA FORM =====
      ...
      // Collect all roll kg, m values, cuon id, and vi tri
      ...
      // Sum all roll kg values for the quantity column and m values for M column
      const totalKg = rollValues.reduce((sum, val) => sum + val.kg, 0);
      const totalM = rollValues.reduce((sum, val) => sum + val.m, 0);
      ...
      if (cuonIdIdx !== -1) {
        const maxIdx = cuonIdIdx;
        while (updatedRow.length <= maxIdx) updatedRow.push('');
        updatedRow[cuonIdIdx] = rollCuonIds.join(', ');
      }
      if (viTriIdx !== -1) {
        const maxIdx = viTriIdx;
        while (updatedRow.length <= maxIdx) updatedRow.push('');
