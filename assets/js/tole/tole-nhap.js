/* =============================================================================
   TOLE-NHAP - SUPABASE VERSION
   Quản lý dữ liệu nhập tole từ Supabase
   Tên bảng: tole-nhap
...
document.addEventListener('submit', async (e) => {
  try {
      ...
      if (window.XgSapLookup) {
        ...
        if (!window._currentSelectedSapRecord && phieuNhapVal) {
          try {
            ...
            console.warn('[tole-nhap] Không thể tra cứu SAP trước khi lưu:', err);
          }
        }
    ...
    // ===== EDIT DATA =====
      ...
      // Lấy dữ liệu cuộn
      const rollKgValues = [], rollMValues = [], rollCuonIds = [], rollViTris = [];
      document.querySelectorAll('#editRollsTableBody tr').forEach(row => {
        const kgInput = row.querySelector('.edit-roll-kg');
        const mInput = row.querySelector('.edit-roll-m');
        if (kgInput && kgInput.value) {
      ...
      const totalM = rollMValues.reduce((sum, m) => sum + m, 0);

      updateData['Số lượng (Kg)'] = totalKg;
      updateData['Số lượng (m)'] = totalM;
