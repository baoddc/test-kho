  ...
  async function fetchAnnouncements() {
    try {
      ...
      const valid = (data || []).filter(item => {
        ...
        // 1. Lọc thông báo cá nhân theo cột target_user
        if (!isAdmin && item.target_user && item.target_user !== '*' && item.target_user !== currentUser) {
          return false;
        }

        // 2. Lọc thông báo cá nhân dạng fallback tag [Gửi username] (nếu DB chưa tạo cột target_user)
        if (!isAdmin && item.title && item.title.startsWith('[Gửi ')) {
          const tagMatch = item.title.match(/^\[Gửi ([^\]]+)\]/);
      ...
      updateBtn.onclick = () => {
        ...
        if (env.isElectron || env.isDesktopPC) {
          const pcLink = (window.APP_DOWNLOAD_LINKS && window.APP_DOWNLOAD_LINKS.pcExe) 
            || 'https://www.dropbox.com/scl/fi/l2nhy7pel593khdommtrx/H-th-ng-Qu-n-l-Kho-Ph-i-Cu-n-DDC-Setup-1.0.7.exe?rlkey=0fxq4od8nm00lfp9rzow2c2zs&st=bzmre4sh&dl=1';
