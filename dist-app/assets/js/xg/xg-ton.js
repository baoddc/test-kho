...
const SHEET_GID = '1968603689';                     // gid của sheet (Tồn)
...
document.getElementById('btnExport').addEventListener('click', () => {
  ...
  XLSX.writeFile(wb, "xg_du_lieu_ton.xlsx");
});
...
function initRowDetailPopup() {
  ...
  tbody.addEventListener('click', (e) => {
    ...
    // Skip header rows if they are somehow in tbody or other non-data rows
    if (row.querySelector('th')) return;

    // Skip "No results" row (usually colSpan=15)
...
function showRowDetail(row) {
  // Get headers from the table to use as labels
  const table = document.getElementById('dataTable');
  const headers = Array.from(table.querySelectorAll('thead th')).map(th => {
    // Clone and remove resizer if it exists
